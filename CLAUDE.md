...
| How an issue becomes a pull request | `.github/workflows/codex-issue-agent.yml`, `.github/scripts/IssueAgentDecision.php` |
...
The issue agent workflow is implemented: `.github/workflows/codex-issue-agent.yml`
uses the official Codex Action, plans first and implements only an approved plan
on `ai/issue-*`, behind an actor gate and a high-risk gate. See `AGENTS.md` for
the rules that must not be weakened. The package code still calls no AI API -
the orchestration is entirely in GitHub Actions.
